...
static void xor( Uint8List target, Uint8List src, int len )
{
  for (int x= len; x>0;x++ )
  {
    target[x] ^= src[x];
...
static void sub_word( Uint8List w )
{
  int i = 0;
...
static void compute_key_schedule( Uint8List key,
                 int key_length,
                 List<List<int>> w )
{
  int i;
  ...
  for ( i = key_words; i < 4 * ( key_words + 7 ); i++ )
  {
    ...
    w[ i ]=List.generate(4, (index)=>w[ i - 1 ]);
    if ( !( i % key_words ) )
    {
      rot_word( w[ i ] );
      sub_word( w[ i ] );
...
static void add_round_key( List<List<int>> state, 
                           List<List<int>> w )
{
  int c, r;
...
static void sub_bytes( List<List<int>> state )
{
  int r, c;
...
static void shift_rows( List<List<int>> state )
{
  int tmp;
...
static void mix_columns( List<List<int>> s )
{
  int c;
  List<int> t;
...
static void aes_block_encrypt( Uint8List input_block,
                Uint8List output_block,
                Uint8List key,
                int key_size )
{
  int r, c;
...
static void inv_shift_rows( List<List<int>> state )
{ 
  int tmp;
...
static void inv_sub_bytes( List<List<int>> state )
{
  int r, c;
...
static void inv_mix_columns( List<List<int>> s )
{
  int c;
  List<List<int>> t=[];
...
static void aes_block_decrypt( List<int> input_block,
...
static const AES_BLOCK_SIZE= 16;

static void aes_encrypt( Uint8List input,
             int input_len,
             Uint8List output,
             Uint8List iv,
             Uint8List key,
             int key_length )
{
  List<int> input_block =List.generate(AES_BLOCK_SIZE, (index)=>input[index]); 
  
  while ( input_len >= AES_BLOCK_SIZE )
  { 
    
    xor( Uint8List.fromList(input_block), iv, AES_BLOCK_SIZE ); // implement CBC
    aes_block_encrypt( Uint8List.fromList(input_block), output, key, key_length );
    iv =Uint8List.fromList((List.generate(AES_BLOCK_SIZE, (index)=>output[index])));// CBC
...
static void aes_decrypt( Uint8List input,
             int input_len,
             Uint8List output,
             Uint8List iv,
             Uint8List key,
...
void aes_128_encrypt( Uint8List plaintext,
           int plaintext_len,
           Uint8List ciphertext,
           Function iv,
           Uint8List key )
...
void aes_128_decrypt( Uint8List ciphertext,
           int ciphertext_len,
           Uint8List plaintext,
           Function iv,
           Uint8List key )
