// Filename: crypto_example.dart
import 'dart:typed_data';
import 'package:hex/hex.dart';
import 'package:collection/collection.dart';
import '../crypto.dart'; // This should point to your working crypto.dart file

// Helper function to compare two byte lists.
bool listsAreEqual(Uint8List a, Uint8List b) {
  return DeepCollectionEquality().equals(a, b);
}

Future<void> main() async {
  print('--- Running QUIC Crypto Example ---');

  // ** CORRECTED TEST VECTORS from RFC 9001 Appendix A **

  // The plain header includes the full unencrypted header up to and including
  // the 4-byte packet number. Its total length is 22 bytes.
  final clientPlainHeader = Uint8List.fromList(
    HEX.decode('c300000001088394c8f03e5157080000449e00000002'),
  );

  // The plain payload is the CRYPTO frame (262 bytes) followed by the
  // correct amount of padding (900 bytes) to match the length specified in the header.
  final clientPlainPayload = Uint8List.fromList(
    HEX.decode(
          '',
        ) +
        List.filled(900, 0), // Correct padding is 900 bytes
  );

  const clientPacketNumber = 2;
  final clientEncryptedPacket = Uint8List.fromList(
    HEX.decode(
      '',
    ),
  );

  print('\n[1] Running Decrypt Test...');
  try {
    final pair = CryptoPair();
    await pair.setupInitial(
      cid: Uint8List.fromList(HEX.decode("8394c8f03e515708")),
      isClient: false,
    );

    // The packet number offset is 18 bytes for this header format
    final (pHeader, pPayload, pNum) = await pair.recv.decryptPacket(
      clientEncryptedPacket,
      18,
      0,
    );

    assert(
      listsAreEqual(pHeader, clientPlainHeader),
      'Decrypted header mismatch!',
    );
    assert(
      listsAreEqual(pPayload, clientPlainPayload),
      'Decrypted payload mismatch!',
    );
    assert(pNum == clientPacketNumber, 'Decrypted packet number mismatch!');

    print('✅ Decrypt Test PASSED');
  } catch (e, st) {
    print('❌ Decrypt Test FAILED: $e');
    print(st);
  }

  print('\n[2] Running Encrypt Test...');
  try {
    final pair = CryptoPair();
    await pair.setupInitial(
      cid: Uint8List.fromList(HEX.decode("8394c8f03e515708")),
      isClient: true,
    );

    final encrypted = await pair.send.encryptPacket(
      clientPlainHeader,
      clientPlainPayload,
      clientPacketNumber,
    );

    assert(
      listsAreEqual(encrypted, clientEncryptedPacket),
      'Encrypted packet mismatch!',
    );

    print('✅ Encrypt Test PASSED');
  } catch (e, st) {
    print('❌ Encrypt Test FAILED: $e');
    print(st);
  }

  print('\n--- Example Finished ---');
}
