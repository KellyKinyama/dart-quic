...
void testReceivingFunctionWithRfcVector() {
  print('\n--- Testing receiving_udp_quic_packet with RFC 9001 Vector ---');

  // 1. Prepare the RFC 9001 Client Initial Packet (Protected)
  // This is the "Expected Packet" from your test vectors
  final rfcPacket = splitHexString(
  ...
  final fromIp = "127.0.0.1";
  final fromPort = 4433;

  // 3. Execute the function
  // try {
  receiving_udp_quic_packet(server, fromIp, fromPort, rfcPacket);

  // 4. Verification
  final expectedDCID = "8394c8f03e515708";
  if (server.connections.containsKey(expectedDCID)) {
    print("✅ SUCCESS: Connection created with RFC DCID: $expectedDCID");

    final conn = server.connections[expectedDCID]!;
    // Check if keys were derived inside set_quic_connection
    if (conn.init_read_key != null) {
      print("✅ SUCCESS: Initial keys derived for RFC connection");
    }
  } else {
    print("❌ FAILED: Connection ID mismatch.");
    print("Actual IDs in server: ${server.connections.keys.toList()}");
  }
  // } catch (e, stack) {
...
void main() {
  testReceivingFunctionWithRfcVector();
}
