...
import 'package:quic_crypto/cipher_suite.dart';
import 'package:quic_crypto/header_protector.dart';

// Helper to create a sealer and opener for tests
(LongHeaderSealer, LongHeaderOpener) getSealerAndOpener(
    CipherSuite cs, Version v) {
  final rand = Random.secure();
  final key = Uint8List.fromList(List.generate(cs.keyLen, (_) => rand.nextInt(256)));
  final hpKey = Uint8List.fromList(List.generate(cs.keyLen, (_) => rand.nextInt(256)));
  final iv = Uint8List.fromList(List.generate(cs.ivLen, (_) => rand.nextInt(256)));

  final aead = cs.aead(key, iv);
  final headerProtector = newHeaderProtector(cs, hpKey, true, v);

  return (_LongHeaderSealer(aead, headerProtector),
      _LongHeaderOpener(aead, headerProtector));
}
    ...
    for (final csId in [0x1301, 0x1302, 0x1303]) {
      group('AEAD for QUIC $v / CipherSuite $csId', () {
        ...
        test('encrypts and decrypts a message', () {
          ...
          expect(
            () => opener.open(Uint8List(0), encrypted, 0x1337,
        ...
        test('encrypts and decrypts header', () {
          ...
          for (var i = 0; i < 20; i++) {
            final sample = Uint8List.fromList(List.generate(16, (_) => rand.nextInt(256)));
            final header = Uint8List.fromList([0xb5, 1, 2, 3, 4, 5, 6, 7, 8, 0xde, 0xad, 0xbe, 0xef]);
