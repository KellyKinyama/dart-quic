...
/// Verifies the entire client-side sealing and protection process using the new builder.
Uint8List testClientInitialProtection() {
  print('\n--- Running Test: Client Initial Packet Protection Vector ---');
  final connID = splitHexString('0x8394c8f03e515708');
  final version = Version.version1;

  // The unprotected header from your test vector, decoded for the builder.
  // We'll use this to extract the original packet number and payload.
  final originalHeader = splitHexString(
    "c300000001088394c8f03e5157080000449e00000002",
  );
  final originalPacketNumber = 2;
  final originalScid = Uint8List(0); // SCID is empty in this test vector
  final originalPayload = splitHexString(
    "060040f1010000ed0303ebf8fa56f129 39b9584a3896472ec40bb863cfd3e868 04fe3a47f06a2b69484c000004130113 02010000c000000010000e00000b6578 616d706c652e636f6dff01000100000a 00080006001d00170018001000070005 04616c706e0005000501000000000033 00260024001d00209370b2c9caa47fba baf4559fedba753de171fa71f50f1ce1 5d43e994ec74d748002b000302030400 0d0010000e0403050306030203080408 050806002d00020101001c0002400100 3900320408ffffffffffffffff050480 00ffff07048000ffff08011001048000 75300901100f088394c8f03e51570806 048000ffff",
  );

  // Call the new general-purpose builder
  final generatedPacket = QuicPacketBuilder.buildClientInitialPacket(
    destinationCid: connID,
    sourceCid: originalScid,
    payload: originalPayload,
    packetNumber: originalPacketNumber,
    version: version,
  );

  // You can now compare 'generatedPacket' against your 'expectedPacket'
  // to confirm everything works as expected.
  print('Packet generated successfully with the new builder!');
  print('Generated packet length: ${generatedPacket.length}');
...
// --- Parsing Logic ---
// void pullQuicHeader(Uint8List packetBytes) {
//   Buffer buf = Buffer(data: packetBytes);
//   final mutablePacket = Uint8List.fromList(packetBytes);
//   final buffer = mutablePacket.buffer;

//   final packetStart = buf.tell();
//   final firstByteView = buf.viewBytes(1);
...
//     final length = buf.pullVarInt();
//     //packetLength=readOffset + length - packetStart;
...
//     final cipherTextOffset = buf.position(offset: currentPosition + 4);
//     final sample = buf.viewBytes(16);
//     // final sample = Uint8List.view(buffer, nextPos, 16);
//     currentPosition = buf.position(offset: pnOffset);
//     final protectedPnBytesView = buf.viewBytes(4);
//     final (_, opener) = newInitialAEAD(
//       dcid,
//       Perspective.server,
//       Version.version1,
...
//     buf.position(offset: cipherTextOffset);
//     final ciphertext = buf.pullBytes(
//       packetLength - cipherTextOffset - pnLength,
//     );
...
void pullQuicHeader(Uint8List packetBytes) {
  ...
  // --- 1. Parse the Header ---
  // We parse sequentially to find the packet number offset (pnOffset).
  final firstByteView = Uint8List.view(packetBytes.buffer, 0, 1);
  final firstByte = buf.pullUint8();

  if ((firstByte & PACKET_LONG_HEADER) == 0) {
    print("Not a long header packet.");
    return;
  }

  buf.pullUint32(); // Version
  final dcidLen = buf.pullUint8();
  final dcid = buf.pullBytes(dcidLen);
  final scidLen = buf.pullUint8();
  buf.pullBytes(scidLen); // SCID

  final tokenLen = buf.pullVarInt();
  buf.pullBytes(tokenLen); // Token

  // This is the total length of the payload (Packet Number + Frames + AEAD Tag)
  final payloadFieldLength = buf.pullVarInt();
  // This is the correct offset where the protected packet number starts.
  final pnOffset = buf.tell();

  // --- 2. Unprotect the Header ---

  // The sample offset is *always* calculated by assuming a 4-byte packet number.
  ...
  // We need a view of the protected packet number bytes. A 4-byte view is safe.
  ...
  // After decryption, the first byte reveals the actual packet number length.
  final pnLength = (firstByteView[0] & 0x03) + 1;
  int wirePn = 0;
  for (int i = 0; i < pnLength; i++) {
    wirePn = (wirePn << 8) | protectedPnBytesView[i];
  ...
  // The payload (ciphertext) starts *immediately after* the actual packet number.
  final payloadOffset = pnOffset + pnLength;

  // The ciphertext's length is the total payload length minus the packet number's length.
  final ciphertextLength = payloadFieldLength - pnLength;

  // The associated data (AD) is the entire unencrypted header, up to the payload.
  final associatedData = Uint8List.view(packetBytes.buffer, 0, payloadOffset);

  // Create a view for the ciphertext.
  final ciphertext = Uint8List.view(
    packetBytes.buffer,
    payloadOffset,
    ciphertextLength,
  ...
  // DEBUG: Critical check of offsets and lengths
  print('DEBUG: Payload (Ciphertext) starts at offset: $payloadOffset');
  print('DEBUG: Associated Data Length: ${associatedData.length}');
  print('DEBUG: Ciphertext Length: ${ciphertext.length}');
