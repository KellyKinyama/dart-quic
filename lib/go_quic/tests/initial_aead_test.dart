...
void initialAeadKeyAndIvDerivation() {
  ...
  for (final tt in tests) {
    ...
    print("Got clientSecret:      $clientSecret");
    print("Expected clientSecret: ${tt['expectedClientSecret']}");
    print("");

    final (key, iv) = computeInitialKeyAndIV(clientSecret, version);
    // expect(key, equals(tt['expectedKey']));
    print("Got key:      $key");
    print("Expected key: ${tt['expectedKey']}");
    print("");
    // expect(iv, equals(tt['expectedIV']));
    print("Got iv:      $iv");
    print("Expected iv: ${tt['expectedIV']}");
    print("");
  }
  // });
  ...
  for (final tt in tests) {
    ...
    print("Got serverSecret:      $serverSecret");
    print("Expected serverSecret: ${tt['expectedServerSecret']}");
    print("");
...
void initialAeadSealingAndOpening() {
  for (final ver in Version.values) {
    // test('seals and opens successfully for $ver', () {
    print('seals and opens successfully for $ver');
    ...
    // );
    print("Got openedClientMessage:      $openedClientMessage");
    print(
      "Expected ClientMessage:       ${Uint8List.fromList('foobar'.codeUnits)}",
    );
    print("");
    ...
    // );
    print("Got openedServerMessage:      $openedServerMessage");
    print(
      "Expected ServerMessage:       ${Uint8List.fromList('raboof'.codeUnits)}",
    );
    print("");
    // });
...
void main() {
  initialAeadKeyAndIvDerivation();
  initialAeadSealingAndOpening();
}
