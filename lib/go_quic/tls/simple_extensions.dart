...
class ExtRenegotiationInfo extends Extension {
  ...
  Uint8List encode() {
    return Uint8List.fromList([
      0,
    ]); // Go version encodes a single byte '0' for length
  }
...
class ExtUseSRTP extends Extension {
  ...
  Uint8List encode() {
    ...
    final lengthBytes = (ByteData(
      2,
    )..setUint16(0, protectionProfiles.length * 2)).buffer.asUint8List();
  ...
  static ExtUseSRTP decode(int extensionLength, Uint8List buf) {
    ...
    for (int i = 0; i < protectionProfilesCount; i++) {
      final protectionProfile = SRTPProtectionProfile.fromInt(
...
class ExtSupportedEllipticCurves extends Extension {
  ...
  Uint8List encode() {
    ...
    for (var c in curves) {
      final curveBytes = (ByteData(
        2,
      )..setUint16(0, c.value)).buffer.asUint8List();
...
class ExtUnknown extends Extension {
  ...
  static ExtUnknown decode(int extensionLength, Uint8List buf) {
    ...
    return ExtUnknown(
      type: ExtensionTypeValue.Unsupported,
