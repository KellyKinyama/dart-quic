...
class NewRenoCongestionController extends AbstractCongestionController implements CongestionController {
    ...
    int kLossReductionFactor = 2;

    // https://tools.ietf.org/html/draft-ietf-quic-recovery-23#appendix-B.1
    // "Minimum congestion window in bytes.  The RECOMMENDED value is 2 * kMaxDatagramSize."
    int kMinimumWindow = 2 * 1200;

    long kMaxDatagramSize = 1200;           // TODO: 1200 is the minimum, actual value can be larger

    long slowStartThreshold = Long.MAX_VALUE;
    Instant congestionRecoveryStartTime;

    NewRenoCongestionController(Logger logger, CongestionControlEventListener eventListener) {
    ...
    synchronized void registerAcked(List<? extends PacketInfo> acknowlegdedPackets) {
        int epsilon = 3;
         bool cwndLimited = congestionWindow - bytesInFlight <= epsilon;
...
//                + " (" + ((congestionWindow - bytesInFlight) / (congestionWindow / 100)) + "%). Cwnd limited? "+ cwndLimited);
    ...
    void fireCongestionEvent(Instant timeSent) {
