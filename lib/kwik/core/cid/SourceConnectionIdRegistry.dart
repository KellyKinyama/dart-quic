...
class SourceConnectionIdRegistry extends ConnectionIdRegistry {

    SourceConnectionIdRegistry(Integer cidLength, Logger logger) {
        super(cidLength, logger);
    }

    ConnectionIdInfo generateNew() {
     ...
     */
     bool registerUsedConnectionId(Uint8List connectionId) {
        if (! Arrays.equals(currentConnectionId, connectionId)) {
            ...
            // Check if new connection id is newly used
             bool wasNew = connectionIds.values().stream()
                    .filter(cid -> Arrays.equals(cid.getConnectionId(), currentConnectionId))
