...
class ConnectionIdManager {
    ...
    static final int MAX_CIDS_PER_CONNECTION = 6;

    final int connectionIdLength;
    final ServerConnectionRegistry connectionRegistry;
    final Sender sender;
    final BiConsumer<Integer, String> closeConnectionCallback;
    final SourceConnectionIdRegistry cidRegistry;
    final DestinationConnectionIdRegistry peerCidRegistry;
    final Uint8List initialConnectionId;
    final Uint8List initialPeerConnectionId;
    ...
    volatile int maxCids = 2;
    /** The maximum number of peer connection IDs this endpoint is willing to maintain; advertised in TP sent by this endpoint */
    volatile int maxPeerCids;
    volatile Uint8List retrySourceCid;
    final Version quicVersion = Version.QUIC_version_1;
     ...
     * @param log  logger
     */
    ConnectionIdManager(Uint8List initialClientCid, Uint8List originalDestinationConnectionId, int connectionIdLength,
     ...
     * @param log  logger
     */
    ConnectionIdManager(Integer connectionIdLength, int maxPeerCids, Sender sender, BiConsumer<Integer, String> closeConnectionCallback, Logger log) {
        this.maxPeerCids = maxPeerCids;
        this.sender = sender;
        cidRegistry = new SourceConnectionIdRegistry(connectionIdLength, log);
        ...
        connectionRegistry = new ServerConnectionRegistry() {   // TODO
            @Override
            void registerConnection(ServerConnectionProxy connection, Uint8List connectionId) {}

            @Override
            void deregisterConnection(ServerConnectionProxy connection, Uint8List connectionId) {}

            @Override
            void registerAdditionalConnectionId(Uint8List currentConnectionId, Uint8List newConnectionId) {}

            @Override
            void deregisterConnectionId(Uint8List connectionId) {}
    ...
    void process(NewConnectionIdFrame frame) {
        ...
        if (!peerCidRegistry.connectionIds.containsKey(frame.getSequenceNr())) {
             bool added = peerCidRegistry.registerNewConnectionId(frame.getSequenceNr(), frame.getConnectionId(), frame.getStatelessResetToken());
            if (! added) {
                // https://www.rfc-editor.org/rfc/rfc9000.html#name-new_connection_id-frames
    ...
    void process(RetireConnectionIdFrame frame, Uint8List destinationConnectionId) {
        ...
        Uint8List retiredCid = cidRegistry.retireConnectionId(sequenceNr);
     ...
     * @param maxPeerCids  the maximum number of peer connection IDs this endpoint is willing to store
     */
    void setMaxPeerConnectionIds(int maxPeerCids) {
        this.maxPeerCids = maxPeerCids;
    }
     ...
     * @param peerCidLimit
     */
    void registerPeerCidLimit(int peerCidLimit) {
        // https://www.rfc-editor.org/rfc/rfc9000.html#name-issuing-connection-ids
     ...
     * @return
     */
    ConnectionIdInfo sendNewCid(int retirePriorTo) {
    ...
    void retransmitFrame(QuicFrame frame) {
     ...
     * @param seqNr
     */
    void sendRetireCid(Integer seqNr) {
        // https://www.rfc-editor.org/rfc/rfc9000.html#name-retransmission-of-informati
    ...
    List<Uint8List> getActivePeerConnectionIds() {
        if (peerCidRegistry != null) {
     ...
     * @return
     */
    Uint8List getCurrentPeerConnectionId() {
        if (peerCidRegistry != null) {
     ...
     */
    Uint8List getInitialConnectionId() {
     ...
     */
     bool validateInitialPeerConnectionId(Uint8List connectionId) {
     ...
     */
    void registerConnectionIdInUse(Uint8List connectionId) {
     ...
     * @param retirePriorTo
     */
    ConnectionIdInfo sendNewConnectionId(int retirePriorTo) {
     ...
     */
    void registerRetrySourceConnectionId(Uint8List connectionId) {
     ...
     */
     bool validateRetrySourceConnectionId(Uint8List connectionId) {
     ...
     * @param connectionId
     */
    void registerInitialPeerCid(Uint8List connectionId) {
     ...
     */
    void setInitialStatelessResetToken(Uint8List statelessResetToken) {
     ...
     * @return
     */
     bool isStatelessResetToken(Uint8List data) {
     ...
     * @return
     */
    Map<Integer, ConnectionIdInfo> getAllConnectionIds() {
     ...
     * @return
     */
    Map<Integer, ConnectionIdInfo> getAllPeerConnectionIds() {
     ...
     */
    void retireConnectionId(Integer sequenceNumber) {
     ...
     * @return
     */
    Uint8List getCurrentConnectionId() {
     ...
     * @return
     */
     bool isActiveCid(Uint8List cid) {
