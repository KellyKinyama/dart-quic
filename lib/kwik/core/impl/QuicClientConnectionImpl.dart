 ...
 */
class QuicClientConnectionImpl extends QuicConnectionImpl implements QuicClientConnection, PacketProcessor, TlsStatusEventHandler, FrameProcessor {

    static final long DEFAULT_CONNECT_TIMEOUT_IN_MILLIS = 10_000;
    static final int DEFAULT_MAX_IDLE_TIMEOUT = 60_000;
    static final int MIN_MAX_IDLE_TIMEOUT = 10;
    static final int MIN_RECEIVER_BUFFER_SIZE = 1500;
    static final long DEFAULT_MAX_STREAM_DATA = 250_000;
    static final int MAX_DATA_FACTOR = 10;
    static final int MAX_OPEN_PEER_INITIATED_BIDI_STREAMS = 3;
    static final int MAX_OPEN_PEER_INITIATED_UNI_STREAMS = 3;
    ...
    static final int MIN_ACTIVE_CONNECTION_ID_LIMIT = 2;
    static final int DEFAULT_ACTIVE_CONNECTION_ID_LIMIT = 2;
    // https://www.rfc-editor.org/rfc/rfc9000.html#name-transport-parameter-definit
    // "Values below 1200 are invalid."
    static final int MIN_MAX_UDP_PAYLOAD_SIZE = 1200;
    static final int DEFAULT_MAX_UDP_PAYLOAD_SIZE = Receiver.MAX_DATAGRAM_SIZE;
    ...
    final  bool usingIPv4;
    final QuicSessionTicket sessionTicket;
    final TlsClientEngine tlsEngine;
    final DatagramSocket socket;
    final InetAddress serverAddress;
    final SenderImpl sender;
    final Receiver receiver;
    volatile PacketParser parser;
    final StreamManager streamManager;
    volatile TransportParameters transportParams;
    final X509Certificate clientCertificate;
    final PrivateKey clientCertificateKey;
    KeyStore keyManager;
    String keyManagerPrivateKeyPassword;
    final ConnectionIdManager connectionIdManager;
    final Version originalVersion;
    final Version preferredVersion;
    final DatagramSocketFactory socketFactory;
    final long connectTimeout;
    final ClientConnectionConfig connectionProperties;
    volatile Uint8List token;
    final CountDownLatch handshakeFinishedCondition = new CountDownLatch(1);
    volatile TransportParameters peerTransportParams;
    KeepAliveActor keepAliveActor;
    String applicationProtocol;
    final List<QuicSessionTicket> newSessionTickets = Collections.synchronizedList(new ArrayList<>());
     bool ignoreVersionNegotiation;
    volatile EarlyDataStatus earlyDataStatus = None;
    final List<TlsConstants.CipherSuite> cipherSuites;

    final GlobalAckGenerator ackGenerator;
    Integer clientHelloEnlargement;
    volatile Thread receiverThread;
    volatile String handshakeError;
    volatile ClientHello originalClientHello;


    QuicClientConnectionImpl(String host, int port, InetTools.IPversionOption ipVersionOption, String applicationProtocol, long connectTimeout,
    ...
    @Override
    PacketFilter createProcessorChain() {
    ...
    Predicate<DatagramPacket> createPacketFilter() {
        return packet -> packet.getAddress().equals(serverAddress) && packet.getPort() == serverPort;
    }

     bool handleUnprotectPacketFailure(ByteBuffer data, Exception unprotectException) {
     ...
     */
    @Override
    synchronized List<QuicStream> connect(List<StreamEarlyData> earlyData) throws IOException {
        ...
        try {
             bool handshakeFinished = handshakeFinishedCondition.await(connectTimeout, TimeUnit.MILLISECONDS);
    ...
    List<QuicStream> sendEarlyData(List<StreamEarlyData> streamEarlyDataList) throws IOException {
    ...
    void startHandshake(String applicationProtocol,  bool withEarlyData) {
    ...
    void discard(PnSpace pnSpace, String reason) {
    ...
    void handleVersionNegotiation(Version packetVersion) {
    ...
    @Override
    ProcessResult process(VersionNegotiationPacket vnPacket, PacketMetaData packetMetaData) {
    ...
    volatile  bool processedRetryPacket = false;
    ...
    ProcessResult process(RetryPacket packet, PacketMetaData metaData) {
        if (packet.validateIntegrityTag(connectionIdManager.getOriginalDestinationConnectionId())) {
            if (!processedRetryPacket) {
                ...
                Uint8List peerConnectionId = packet.getSourceConnectionId();
    ...
    @Override
    void immediateCloseWithError(long error, ErrorType errorType, String errorReason) {
    ...
    @Override
    void cryptoProcessingErrorOcurred(Exception exception) {
    ...
    @Override
    void peerClosedWithError(ConnectionCloseFrame closeFrame) {
    ...
    TransportParameters getTransportParameters() {
    ...
    @Override
    void validateTransportParameters(TransportParameters transportParameters) throws TransportError {
    ...
    void setZeroRttTransportParameters(TransportParameters rememberedTransportParameters) {
    ...
    void verifyVersionNegotiation(TransportParameters transportParameters) {
     ...
     bool verifyConnectionIds(TransportParameters transportParameters) {
        // https://www.rfc-editor.org/rfc/rfc9000.html#section-7.3
    ...
    Uint8List nextDestinationConnectionId() {
        Uint8List newConnectionId = connectionIdManager.nextPeerId();
     ...
     bool checkForStatelessResetToken(ByteBuffer data) {
        Uint8List tokenCandidate = new byte[16];
        data.position(data.limit() - 16);
        data.get(tokenCandidate);
         bool isStatelessReset = connectionIdManager.isStatelessResetToken(tokenCandidate);
        return isStatelessReset;
    }

    Uint8List[] newConnectionIds(int count, int retirePriorTo) {
        Uint8List[] newConnectionIds = new byte[count][];
    ...
    void retireDestinationConnectionId(Integer sequenceNumber) {
    ...
    @Override
    GlobalAckGenerator getAckGenerator() {
        return ackGenerator;
    }

    @Override
    TlsClientEngine getTlsEngine() {
        return tlsEngine;
    }
    ...
    @Override
    ConnectionIdManager getConnectionIdManager() {
        return connectionIdManager;
    }

    @Override
    int getSourceConnectionIdLength() {
    ...
    Map<Integer, ConnectionIdInfo> getSourceConnectionIds() {
    ...
    Map<Integer, ConnectionIdInfo> getDestinationConnectionIds() {
        return connectionIdManager.getAllPeerConnectionIds();
    }

    @Override
    void setPeerInitiatedStreamCallback(Consumer<QuicStream> streamProcessor) {
    ...
    void addNewSessionTicket(NewSessionTicket tlsSessionTicket) {
    ...
    EarlyDataStatus getEarlyDataStatus() {
        return earlyDataStatus;
    }

    void setEarlyDataStatus(EarlyDataStatus earlyDataStatus) {
    ...
    void trustAnyServerCertificate() {
    ...
    void setTrustStore(KeyStore customTrustStore) throws KeyStoreException {
        try {
    ...
    void setKeyManager(KeyStore keyManager) {
        this.keyManager = keyManager;
    }

    void setKeyManagerPrivateKeyPassword(String keyPassword) {
        this.keyManagerPrivateKeyPassword = keyPassword;
    }

    void enableQuantumReadinessTest(int nrDummyBytes) {
    ...
    static ExtendedBuilder newExtendedBuilder() {
        return new ExtendedBuilder();
    }

    static class BuilderImpl implements Builder {

        ClientConnectionConfig connectionProperties = new ClientConnectionConfig();
        String host;
        int port;
        InetTools.IPversionOption ipVersionOption;
        QuicSessionTicket sessionTicket;
        QuicVersion quicVersion = QuicVersion.V1;
        QuicVersion preferredVersion;
        Logger log = new NullLogger();
        String proxyHost;
        Path secretsFile;
        Integer initialRtt;
        Integer connectionIdLength;
        List<TlsConstants.CipherSuite> cipherSuites = new ArrayList<>();
         bool omitCertificateCheck;
        Integer quantumReadinessTest;
        X509Certificate clientCertificate;
        PrivateKey clientCertificateKey;
        DatagramSocketFactory socketFactory;
        long connectTimeoutInMillis = DEFAULT_CONNECT_TIMEOUT_IN_MILLIS;
        String applicationProtocol = "";
        KeyStore customTrustStore;
        KeyStore keyManager;
        String keyPassword;
         bool enableDatagramExtension;
        ...
        @Override
        Builder sessionTicket(Uint8List ticketData) {
     ...
     */
    static class ExtendedBuilder extends BuilderImpl implements Builder {

        ExtendedBuilder activeConnectionIdLimit(int limit) {
         ...
         * @param maxSize
         */
        void maxUdpPayloadSize(int maxSize) {
        ...
        void useStrictSmallestAllowedMaximumDatagramSize() {
         ...
         * @param enforce
         */
        void enforceMaxUdpPayloadSize( bool enforce) {
