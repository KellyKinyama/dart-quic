...
import "package:dart_quic/kwik/core/frame/QuicFrame.dart";
import "package:dart_quic/kwik/core/frame/Range.dart";
import "package:dart_quic/kwik/core/common/PnSpace.dart";
import "package:dart_quic/kwik/core/impl/Version.dart";
...
class AckGenerator {
    ...
    final Timer clock;
    final Version quicVersion = Version.getDefault();
    final PnSpace pnSpace;
    final Sender sender;
    List<Range> rangesToAcknowledge = new ArrayList<>();
     bool newPacketsToAcknowledge;
    Instant newPacketsToAcknowlegdeSince;
    Map<Long, AckFrame> ackSentWithPacket = new HashMap<>();
    int acksNotSend = 0;

    AckGenerator(PnSpace pnSpace, Sender sender) {
        this(Clock.systemUTC(), pnSpace, sender);
    }

    AckGenerator(Clock clock, PnSpace pnSpace, Sender sender) {
        this.clock = clock;
        this.pnSpace = pnSpace;
        this.sender = sender;
    }

    synchronized  bool hasAckToSend() {
        return !rangesToAcknowledge.isEmpty();
    }

    synchronized  bool hasNewAckToSend() {
        return newPacketsToAcknowledge;
    }

    synchronized void packetReceived(QuicPacket packet) {
     ...
     * @param receivedAck
     */
    synchronized void process(QuicFrame receivedAck) {
     ...
     * @return
     */
    synchronized Optional<AckFrame> generateAckForPacket(long packetNumber) {
    ...
    synchronized Optional<AckFrame> generateAck() {
        int delay = 0;
    ...
    synchronized void registerAckSendWithPacket(AckFrame ackFrame, long packetNumber) {
