 ...
 */
class QuicTransportParametersExtension extends Extension {

    // https://www.rfc-editor.org/rfc/rfc9221.html#name-quic-transport-parameter
    static final int MAX_DATAGRAM_FRAME_SIZE = 0x20;

    static final int MINIMUM_EXTENSION_LENGTH = 2;
    static final int CODEPOINT_IETFDRAFT = 0xffa5;
    static final int CODEPOINT_V1 = 0x39;

    final Version quicVersion;
    Role senderRole;
    Uint8List data;
    TransportParameters params;
    Integer discardTransportParameterSize;


    static  bool isCodepoint(Version quicVersion, int extensionType) {
    ...
    QuicTransportParametersExtension(Version quicVersion) {
     ...
     * @param senderRole
     */
    QuicTransportParametersExtension(Version quicVersion, TransportParameters params, Role senderRole) {
        this.quicVersion = quicVersion;
        this.params = params;
    ...
    void addDiscardTransportParameter(int parameterSize) {
    ...
    QuicTransportParametersExtension parse(ByteBuffer buffer, Role senderRole, Logger log) throws TlsProtocolException {
    ...
    void parseTransportParameter(ByteBuffer buffer, Set<Long> parsedParameters, Logger log) throws DecodeErrorException, InvalidIntegerEncodingException, TransportError {
        ...
        if (parameterId == original_destination_connection_id.value) {
            Uint8List destinationCid = new byte[size];
            ...
            Uint8List resetToken = new byte[16];
            ...
            Uint8List initialSourceCid = new byte[size];
            ...
            Uint8List retrySourceCid = new byte[size];
    ...
    void parsePreferredAddress(ByteBuffer buffer, Logger log) throws DecodeErrorException {
        try {
            TransportParameters.PreferredAddress preferredAddress = new TransportParameters.PreferredAddress();

            Uint8List ip4 = new byte[4];
            buffer.get(ip4);
    ...
    void addTransportParameter(ByteBuffer buffer, QuicConstants.TransportParameterId id, long value) {
        addTransportParameter(buffer, id.value, value);
    }

    void addTransportParameter(ByteBuffer buffer, QuicConstants.TransportParameterId id) {
        VariableLengthInteger.encode(id.value, buffer);
        int valueLength = 0;
        VariableLengthInteger.encode(valueLength, buffer);
    }

    void addTransportParameter(ByteBuffer buffer, int id, long value) {
    ...
    void addTransportParameter(ByteBuffer buffer, QuicConstants.TransportParameterId id, Uint8List value) {
        addTransportParameter(buffer, id.value, value);
    }

    void addTransportParameter(ByteBuffer buffer, int id, Uint8List value) {
        VariableLengthInteger.encode(id, buffer);
