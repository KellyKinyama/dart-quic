...
class SendBuffer {
    ...
    final Queue<ByteBuffer> sendQueue;
    final ByteBuffer END_OF_STREAM_MARKER = ByteBuffer.allocate(0);
    final int maxBufferSize;
    final AtomicInteger bufferedBytes;
    final ReentrantLock bufferLock;
    final Condition notFull;
    volatile Thread blockingWriterThread;


    SendBuffer(Integer sendBufferSize) {
     ...
     */
    void write(Uint8List data, int off, int len) throws IOException, InterruptedException {
    ...
    StreamFrame getStreamFrame(Version quicVersion, int streamId, long currentOffset, int maxBytesToSend) {
        int nrOfBytes = 0;
        Uint8List dataToSend = new byte[maxBytesToSend];
