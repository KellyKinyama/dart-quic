...
class StreamManager {
    ...
    static final Consumer<QuicStream> NO_OP_CONSUMER = (stream) -> {};

    final Map<Integer, QuicStreamImpl> streams;
    ...
    final Role role;
    final Logger log;
    volatile ConnectionConfig config;
    volatile int currentUnidirectionalStreamIdLimit;
    volatile int currentBidirectionalStreamIdLimit;
    volatile Consumer<QuicStream> peerInitiatedStreamCallback;
    volatile Long maxStreamsAcceptedByPeerBidi;
    volatile Long maxStreamsAcceptedByPeerUni;
    final Semaphore openBidirectionalStreams;
    final Semaphore openUnidirectionalStreams;
    volatile  bool maxOpenStreamsUniUpdateQueued;
    volatile  bool maxOpenStreamsBidiUpdateQueued;
    volatile long flowControlMax;
    long flowControlLastAdvertised;
    long flowControlIncrement;
    final ReentrantLock maxOpenStreamsUpdateLock;
    final ReentrantLock updateFlowControlLock;
    final AtomicInteger nextStreamIdBidirectional;
    final AtomicInteger nextStreamIdUnidirectional;
    volatile int nextPeerInitiatedUnidirectionalStreamId;
    volatile int nextPeerInitiatedBidirectionalStreamId;
    long cumulativeReceiveOffset;
    long absoluteUnidirectionalStreamIdLimit;
    long absoluteBidirectionalStreamIdLimit;
     ...
     */
    StreamManager(QuicConnectionImpl quicConnection, Role role, Logger log, ConnectionConfig config, ExecutorService callbackExecutor) {
     ...
     * @return
     */
    int computeMaxStreamIdLimit(int maxStreams, Role peerRole,  bool bidirectional) {
        if (maxStreams < 0) {
            return 0;
        }
    ...
    QuicStream createStream( bool bidirectional, long timeout, TimeUnit timeoutUnit) throws TimeoutException {
    ...
    QuicStreamImpl createStream( bool bidirectional, long timeout, TimeUnit unit, QuicStreamSupplier streamFactory) throws TimeoutException {
        try {
             bool acquired;
            if (bidirectional) {
     ...
     * @return
     */
    EarlyDataStream createEarlyDataStream( bool bidirectional) {
    ...
    int generateStreamId( bool bidirectional) {
        if (bidirectional) {
    ...
    void setFlowController(FlowControl flowController) {
        this.flowController = flowController;
    }

    void process(StreamFrame frame) throws TransportError {
    ...
    QuicStreamImpl createPeerInitiatedStream(int requestedStreamId) throws TransportError {
    ...
    void createPeerInitiatedStreams(int requestedStreamId, int nextStreamId, Runnable nextStreamIdUpdate) throws TransportError {
    ...
    void process(StopSendingFrame stopSendingFrame) {
    ...
    void process(ResetStreamFrame resetStreamFrame) throws TransportError {
    ...
    void updateConnectionFlowControl(int size) {
        try {
    ...
    void checkConnectionFlowControl(QuicStreamImpl receivingStream, StreamFrame frame) throws TransportError {
     ...
     bool isNewPeerInitiated(int streamId) {
    ...
    void increaseMaxOpenStreams(int streamId) {
    ...
    QuicFrame createMaxStreamsUpdateUni(int maxFrameSize) {
        if (maxFrameSize < 9) {
    ...
    QuicFrame createMaxStreamsUpdateBidi(int maxFrameSize) {
        if (maxFrameSize < 9) {
            throw new ImplementationError();
        }
        try {
    ...
    void retransmitMaxStreams(QuicFrame frame) {
     ...
     bool isPeerInitiated(int streamId) {
        return streamId % 2 == (role == Role.Client? 1 : 0);
    }

     bool isUni(int streamId) {
        return streamId % 4 > 1;
    }

     bool isBidi(int streamId) {
        return streamId % 4 < 2;
    }

    void process(MaxStreamsFrame frame) {
    ...
    long getMaxBidirectionalStreams() {
        return maxStreamsAcceptedByPeerBidi;
    }

    long getMaxUnidirectionalStreams() {
        return maxStreamsAcceptedByPeerUni;
    }

    long getMaxUnidirectionalStreamBufferSize() {
        return config.maxUnidirectionalStreamBufferSize();
    }

    long getMaxBidirectionalStreamBufferSize() {
        return config.maxBidirectionalStreamBufferSize();
    }

    void setDefaultUnidirectionalStreamReceiveBufferSize(long newSize) {
        config = ConnectionConfigImpl.cloneWithMaxUnidirectionalStreamReceiveBufferSize(config, newSize);
    }

    void setDefaultBidirectionalStreamReceiveBufferSize(long newSize) {
