 ...
 */
class ReceiveBufferImpl implements ReceiveBuffer {

    static final int DEFAULT_MAX_COMBINED_FRAME_SIZE = 5120;

    final NavigableSet<StreamElement> outOfOrderFrames = new ConcurrentSkipListSet<>();
    final Queue<StreamElement> contiguousFrames = new ConcurrentLinkedQueue<>();
    volatile long contiguousUpToOffset = 0;
    volatile long readUpToOffset = 0;
    volatile long streamEndOffset = -1;
    volatile long bufferedOutOfOrderData;
    final int maxCombinedFrameSize;
    volatile  bool discarded;
     ...
     */
    ReceiveBufferImpl(int maxCombinedFrameSize) {
    ...
    long bufferedOutOfOrderData() {
        return bufferedOutOfOrderData;
    }

    void addWithoutOverlap(StreamElement frame) {
    ...
    static  bool overlapping(StreamElement frame1, StreamElement frame2) {
    ...
    static StreamElement combine(StreamElement frame1, StreamElement frame2) {
        ...
        Uint8List combinedData = new byte[newLength];
    ...
    static StreamElement shrinkFrame(StreamElement frame, long newStartOffset, long newUpToOffset) {
        ...
        Uint8List limitedData = new byte[newLength];
    ...
    static  bool contains(StreamElement containing, StreamElement contained) {
    ...
    String toDebugString(int maxElements) {
    ...
    int countOverlap(Iterator<StreamElement> iterator) {
        int overlap = 0;
        if (iterator.hasNext()) {
    ...
    static class SimpleStreamElement implements StreamElement {
        ...
        SimpleStreamElement(long offset, Uint8List data,  bool isFinal) {
            this.offset = offset;
            this.data = data;
