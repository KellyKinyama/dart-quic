...
class StreamOutputStreamImpl extends StreamOutputStream implements FlowControlUpdateListener {
    ...
    static final int MIN_FRAME_SIZE = 1 + 8 + 8 + 2 + 1;
    ...
    final RetransmitBuffer retransmitBuffer;
    ...
    volatile  bool sendRequestQueued;
    // Reset indicates whether the OutputStream has been reset.
    volatile  bool reset;
    volatile long resetErrorCode;
    ...
    @Override
    void write(Uint8List data, int off, int len) throws IOException {
        checkState();
        try {
    ...
    @Override
    void write(int dataByte) throws IOException {
        // Terrible for performance of course, but that is calling this method anyway.
        Uint8List data = new Uint8List{(byte) dataByte};
        write(data, 0, 1);
    }
    ...
    @Override
    void streamNotBlocked(int streamId) {
     ...
     * @return
     */
    QuicFrame sendBlockReason(int maxFrameSize) {
    ...
    void retransmitSendBlockReason(QuicFrame quicFrame) {
        quicStream.connection.send(this::sendBlockReason, StreamDataBlockedFrame.getMaxSize(quicStream.streamId), App, this::retransmitSendBlockReason, true);
    }

    void retransmitStreamFrame(QuicFrame frame) {
    ...
    QuicFrame createResetFrame(int maxFrameSize) {
        assert (reset == true);
        return new ResetStreamFrame(quicStream.streamId, resetErrorCode, currentOffset);
    }

    void retransmitResetFrame(QuicFrame frame) {
