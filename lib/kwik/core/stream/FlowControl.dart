...
import static tech.kwik.core.QuicConstants.TransportErrorCode.STREAM_STATE_ERROR;
...
class FlowControl {
    ...
    final long initialMaxStreamDataBidiLocal;
    ...
    final long initialMaxStreamDataBidiRemote;
    ...
    //  streams."
    final long initialMaxStreamDataUni;
    ...
    long maxDataAssigned;
    // The maximum amount of data that a stream would be allowed to send (to the peer), ignoring possible connection limit
    Map<Integer, Long> maxStreamDataAllowed;
    // The maximum amount of data that is already assigned to a stream (i.e. already sent, or upon being sent)
    Map<Integer, Long> maxStreamDataAssigned;
    final Logger log;
    final Map<Integer, FlowControlUpdateListener> streamListeners;
    int maxOpenedStreamId;


    FlowControl(Role role, long initialMaxData, long initialMaxStreamDataBidiLocal, long initialMaxStreamDataBidiRemote, long initialMaxStreamDataUni) {
    ...
    FlowControl(Role role, long initialMaxData, long initialMaxStreamDataBidiLocal, long initialMaxStreamDataBidiRemote, long initialMaxStreamDataUni, Logger log) {
        this.role = role;
     ...
     */
    long increaseFlowControlLimit(QuicStream stream, long requestedLimit) {
     ...
     * @return
     */
    long getFlowControlLimit(QuicStream stream) {
        synchronized (this) {
     ...
     * @return
     */
    BlockReason getFlowControlBlockReason(QuicStream stream) {
        int streamId = stream.getStreamId();
     ...
     */
    synchronized void updateInitialValues(TransportParameters peerTransportParameters) {
        if (role == Role.Server) {
    ...
    void register(QuicStream stream, FlowControlUpdateListener listener) {
        streamListeners.put(stream.getStreamId(), listener);
    }

    void unregister(QuicStream stream) {
    ...
    void streamClosed(QuicStream stream) {
        int streamId = stream.getStreamId();
        synchronized (this) {
    ...
    long determineInitialMaxStreamData(QuicStream stream) {
     ...
     * @return
     */
    long currentStreamCredits(QuicStream stream) {
        int streamId = stream.getStreamId();
    ...
    void process(MaxStreamDataFrame frame) throws TransportError {
        synchronized (this) {
            ...
            if (maxStreamDataAllowed.containsKey(streamId)) {
                ...
                if (maxStreamData > maxStreamDataAllowed.get(streamId)) {
                     bool streamWasBlocked = maxStreamDataAssigned.get(streamId).longValue() == maxStreamDataAllowed.get(streamId).longValue()
     ...
     bool locallyInitiated(int streamId) {
        if (role == Role.Client) {
            return streamId % 2 == 0;
        }
        else {
