...
class Receiver {
    ...
    static final int MAX_DATAGRAM_SIZE = 1500;

    volatile DatagramSocket socket;
    final Logger log;
    final Consumer<Throwable> abortCallback;
    final Predicate<DatagramPacket> packetFilter;
    final Thread receiverThread;
    final BlockingQueue<RawPacket> receivedPacketsQueue;
    volatile  bool isClosing = false;
    volatile  bool changing = false;

    Receiver(DatagramSocket socket, Logger log, Consumer<Throwable> abortCallback) {
        this(socket, log, abortCallback, d -> true);
    }

    Receiver(DatagramSocket socket, Logger log, Consumer<Throwable> abortCallback, Predicate<DatagramPacket> packetFilter) {
    ...
    void run() {
        ...
        try {
            while (! isClosing) {
                Uint8List receiveBuffer = new byte[MAX_DATAGRAM_SIZE];
