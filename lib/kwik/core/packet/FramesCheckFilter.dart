...
class FramesCheckFilter extends BasePacketFilter {

    FramesCheckFilter(PacketFilter next) {
        super(next);
    }

    FramesCheckFilter(PacketFilter next, Logger log) {
        super(next, log);
    }
    ...
    void checkFrames(InitialPacket packet) throws TransportError {
    ...
    void checkFrames(ZeroRttPacket packet) throws TransportError {
        if (packet.getFrames().stream()
                .anyMatch(
    ...
    void checkFrames(HandshakePacket packet) throws TransportError {
        if (! packet.getFrames().stream()
                .allMatch(
                        // ! allMatch => list frames that are permitted
                        frame -> frame instanceof Padding ||
    ...
    void checkFrames(ShortHeaderPacket packet) {
        // Intentionally left empty: all frames are permitted
    }

    void checkNotEmpty(QuicPacket packet) throws TransportError {
