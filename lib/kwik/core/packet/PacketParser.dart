...
abstract class PacketParser {
    ...
    VersionHolder quicVersion;
    final int cidLength;
    PacketFilter processorChain;
    Logger log;
    final Role role;
    long[] largestPacketNumber;
    BiFunction<ByteBuffer, Exception, Boolean> handleUnprotectPacketFailureFunction;


    PacketParser(ConnectionSecrets secrets, VersionHolder quicVersion, int cidLength, PacketFilter processor, Role role, Logger logger) {
        this(secrets, quicVersion, cidLength, processor, null, role, logger);
    }

    PacketParser(ConnectionSecrets secrets, VersionHolder quicVersion, int cidLength, PacketFilter processor,
    ...
    void parseAndProcessPackets(ByteBuffer data, PacketMetaData metaData) throws TransportError {
    ...
    QuicPacket parsePacket(ByteBuffer data) throws MissingKeysException, DecryptionException, InvalidPacketException, TransportError {
        data.mark();
        if (data.remaining() < 2) {
    ...
    abstract Aead getAead(QuicPacket packet, ByteBuffer data) throws MissingKeysException, InvalidPacketException, TransportError;
     ...
     */
    QuicPacket createLongHeaderPacket(byte flags, ByteBuffer data) throws InvalidPacketException {
