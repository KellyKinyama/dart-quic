 ...
 */
class DropDuplicatePacketsFilter extends BasePacketFilter {
    ...
    final WindowBasedProcessedPacketChecker[] packetNumberSpace;

    DropDuplicatePacketsFilter(PacketFilter next) {
        this(next, DEFAULT_WINDOW_SIZE_INITIAL, DEFAULT_WINDOW_SIZE_HANDSHAKE, DEFAULT_WINDOW_SIZE_APP);
    }

    DropDuplicatePacketsFilter(PacketFilter next, int... windowSizes) {
        super(next);
    ...
    class WindowBasedProcessedPacketChecker {

        final int windowSize;
        long[] processedPackets;

        WindowBasedProcessedPacketChecker(int windowSize) {
            this.windowSize = windowSize;
            this.processedPackets = new long[windowSize];
            Arrays.fill(processedPackets, -1);
        }

         bool checkPacketNumber(QuicPacket packet) {
