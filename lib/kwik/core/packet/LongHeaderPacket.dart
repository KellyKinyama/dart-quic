...
abstract class LongHeaderPacket extends QuicPacket {
    ...
    static int MIN_PACKET_LENGTH = 1 +  4 +     1 +      0 +  1 +      0 +  1 +    1 +    1;

    Uint8List sourceConnectionId;

    static  bool isLongHeaderPacket(byte flags, Version quicVersion) {
        return (flags & 0b1100_0000) == 0b1100_0000;
    }

    static Class determineType(byte flags, Version version) {
     ...
     * @param quicVersion
     */
    LongHeaderPacket(Version quicVersion) {
     ...
     * @param frame
     */
    LongHeaderPacket(Version quicVersion, Uint8List sourceConnectionId, Uint8List destConnectionId, QuicFrame frame) {
        this.quicVersion = quicVersion;
     ...
     * @param frames
     */
    LongHeaderPacket(Version quicVersion, Uint8List sourceConnectionId, Uint8List destConnectionId, List<QuicFrame> frames) {
        if (frames == null) {
    ...
    Uint8List generatePacketBytes(Aead aead) {
        ...
        Uint8List packetBytes = new byte[packetBuffer.position()];
        packetBuffer.rewind();
    ...
    void generateFrameHeaderInvariant(ByteBuffer packetBuffer) {
    ...
    abstract void generateAdditionalFields(ByteBuffer packetBuffer);

    abstract int estimateAdditionalFieldsLength();

    void addLength(ByteBuffer packetBuffer, int packetNumberLength, int payloadSize) {
    ...
    @Override
    void parse(ByteBuffer buffer, Aead aead, long largestPacketNumber, Logger log, int sourceConnectionIdLength) throws DecryptionException, InvalidPacketException, TransportError {
    ...
    abstract void parseAdditionalFields(ByteBuffer buffer) throws InvalidPacketException;
}
