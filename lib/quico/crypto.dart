...
// ({Uint8List Function(Uint8List prk, Uint8List info, int outputLength) hkdfExpand, Uint8List Function(Uint8List prk, {required Uint8List salt}) hkdf_extract, Uint8List Function(Uint8List prk, Uint8List info) hmac, Uint8List Function(Uint8List prk, Uint8List info) sha256}) nobleHashes = (
//   hmac: hmacSha256,
...
({int keylen, int ivlen, HashAlgorthm hash}) get_cipher_info(cipher_suite) {
  switch (cipher_suite) {
    case 0x1301: // TLS_AES_128_GCM_SHA256
      return ( keylen: 16, ivlen: 12, hash:  HashAlgorthm.sha256 );
    case 0x1302: // TLS_AES_256_GCM_SHA384
      return ( keylen: 32, ivlen: 12, hash: HashAlgorthm.sha384 );
    case 0x1303: // TLS_CHACHA20_POLY1305_SHA256
      return ( keylen: 32, ivlen: 12, hash: HashAlgorthm.sha256 );
    default:
...
Uint8List build_server_hello(Uint8List server_random, Uint8List public_key, Uint8List session_id, int cipher_suite, int group) {
    List<int> legacy_version = [0x03, 0x03];
    List<int> random = Uint8List.fromList(server_random);
    List<int> session_id_bytes = Uint8List.fromList(session_id);
    int session_id_length = session_id_bytes.length & 0xff;

    var cipher_suite_bytes = [(cipher_suite >> 8) & 0xff, cipher_suite & 0xff];
    List<int> compression_method = [0x00];

    Uint8List key = Uint8List.fromList(public_key);
    List<int> key_length = [(key.length >> 8) & 0xff, key.length & 0xff];
    var group_bytes = [(group >> 8) & 0xff, group & 0xff];
    List<int> key_exchange = [...group_bytes, ...key_length, ...key];
    List<int> key_share_extension  = (){
        List<int> extension_type = [0x00, 0x33];
        List<int> extension_length = [(key_exchange.length >> 8) & 0xff, key_exchange.length & 0xff];
        return [...extension_type, ...extension_length, ...key_exchange];
    }();

    List<int> supported_versions_extension = [
        0x00, 0x2b,
        0x00, 0x02,
        0x03, 0x04
    ];

    List<int> params_bytes = [
    ...
    List<int> extensions_length = [(extensions.length >> 8) & 0xff, extensions.length & 0xff];
