...
class Aead {
  ...
  Uint8List decrypt(Uint8List data, Uint8List associatedData, int packetNumber) {
    final nonce = _createNonce(packetNumber);
    _cipher.init(false, AEADParameters(KeyParameter(_key), _tagLength * 8, nonce, associatedData));
  ...
  Uint8List encrypt(Uint8List data, Uint8List associatedData, int packetNumber) {
    final nonce = _createNonce(packetNumber);
    _cipher.init(true, AEADParameters(KeyParameter(_key), _tagLength * 8, nonce, associatedData));
...
class HeaderProtection {
  ...
  static BlockCipher _createCipher(String cipherName) {
    switch (cipherName) {
      ...
      case 'chacha20':
         ...
         return ChaChaEngine();
      default:
        throw ArgumentError('Unsupported header protection cipher: $cipherName');
    }
  }

  Uint8List _createMask(Uint8List sample) {
    if (_isChaCha20) {
      final chacha = _cipher as ChaChaEngine;
  ...
  Uint8List apply(Uint8List header, Uint8List payload) {
    ...
    if ((protectedPacket[0] & 0x80) != 0) { // Long Header
  ...
  (Uint8List, int) remove(Uint8List packet, int encryptedOffset) {
    final sample = packet.sublist(encryptedOffset + 4, encryptedOffset + 4 + 16);
    final mask = _createMask(sample);
    
    final plainHeader = packet.sublist(0, encryptedOffset);
    
    // Remove mask from first byte
    if ((plainHeader[0] & 0x80) != 0) { // Long Header
    ...
    plainHeader.setRange(pnOffset, pnOffset + pnLength, packet.sublist(encryptedOffset, encryptedOffset + pnLength));
