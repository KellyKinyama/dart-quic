...
class CryptoContext {
  ...
  Future<void> setup({
    ...
    aead = Aead(
      ...
      await iv.extractBytes(),
    );
    hp = HeaderProtection(hpCipherName, await hpKey.extractBytes());
  }
...
Future<(SecretKey, SecretKey, SecretKey)> deriveKeyIvHp({
  ...
  final keyLabel = version == QuicProtocolVersion.version2
      ? 'quicv2 key'.codeUnits
      : 'quic key'.codeUnits;
  final ivLabel = version == QuicProtocolVersion.version2
      ? 'quicv2 iv'.codeUnits
      : 'quic iv'.codeUnits;
  final hpLabel = version == QuicProtocolVersion.version2
      ? 'quicv2 hp'.codeUnits
      : 'quic hp'.codeUnits;
