...
class VariableLengthInteger {
  ...
  factory VariableLengthInteger.decode(Uint8List bytes, int offset) {
    ...
    int lengthIndicator = (firstByte >> 6); // Extract the two most significant bits
    ...
    switch (lengthIndicator) {
      ...
      case 1: // 2-byte encoding
        ...
        if (offset + 1 >= bytes.length) {
          throw const FormatException('Cannot decode 2-byte VLI: Insufficient data.');
        }
      ...
      case 2: // 4-byte encoding
        ...
        if (offset + 3 >= bytes.length) {
          throw const FormatException('Cannot decode 4-byte VLI: Insufficient data.');
        }
        value = (firstByte & 0x3F) << 24 | bytes[offset + 1] << 16 | bytes[offset + 2] << 8 | bytes[offset + 3];
        break;
      case 3: // 8-byte encoding
        byteLength = 8;
        if (offset + 7 >= bytes.length) {
          throw const FormatException('Cannot decode 8-byte VLI: Insufficient data.');
        }
  ...
  Uint8List encode() {
    ...
    } else if (value < 4611686018427387904) { // 2^62 - 1, max value for 8-byte VLI
      final ByteData bd = ByteData(8);
      bd.setUint64(0, value);
      final Uint8List bytes = bd.buffer.asUint8List();
      return Uint8List.fromList([
        0xC0 | (bytes[0] & 0x3F), bytes[1], bytes[2], bytes[3],
        bytes[4], bytes[5], bytes[6], bytes[7]
      ]);
    } else {
      throw ArgumentError('Value too large for QUIC Variable-Length Integer encoding: $value');
    }
  }
}
...
abstract class QuicFrame {
  ...
  static QuicFrame parse(Uint8List bytes, int offset) {
    if (bytes.isEmpty || offset >= bytes.length) {
      throw const FormatException('Cannot parse QUIC frame: Insufficient data.');
    }

    final VariableLengthInteger frameTypeVli = VariableLengthInteger.decode(bytes, offset);
    final int frameType = frameTypeVli.value;
    offset += frameTypeVli.byteLength;

    switch (frameType) {
      case 0x01: // PING frame type
        return PingFrame.parse(bytes, offset - frameTypeVli.byteLength); // Pass the entire frame starting from type byte
      ...
      case 0x09: // RETIRE_CONNECTION_ID frame type
        return RetireConnectionIdFrame.parse(bytes, offset - frameTypeVli.byteLength);
      ...
      case 0x0F: // STREAM_DATA_BLOCKED frame type
        return StreamDataBlockedFrame.parse(bytes, offset - frameTypeVli.byteLength);
      ...
      case 0x06: // STREAMS_BLOCKED frame type (unidirectional)
        return StreamsBlockedFrame.parse(bytes, offset - frameTypeVli.byteLength);
...
class PingFrame extends QuicFrame {
  ...
  factory PingFrame.parse(Uint8List bytes, int offset) {
    ...
    if (frameTypeVli.value != 0x01) {
      throw FormatException('Invalid frame type for PingFrame: 0x${frameTypeVli.value.toRadixString(16)}');
    }
...
class ResetStreamFrame extends QuicFrame {
  ...
  factory ResetStreamFrame.parse(Uint8List bytes, int offset) {
    ...
    final VariableLengthInteger frameTypeVli = VariableLengthInteger.decode(bytes, currentOffset);
    if (frameTypeVli.value != 0x04) {
      throw FormatException('Invalid frame type for ResetStreamFrame: 0x${frameTypeVli.value.toRadixString(16)}');
    }
    currentOffset += frameTypeVli.byteLength;

    final VariableLengthInteger streamIdVli = VariableLengthInteger.decode(bytes, currentOffset);
    currentOffset += streamIdVli.byteLength;

    final VariableLengthInteger errorCodeVli = VariableLengthInteger.decode(bytes, currentOffset);
    currentOffset += errorCodeVli.byteLength;

    final VariableLengthInteger finalSizeVli = VariableLengthInteger.decode(bytes, currentOffset);
  ...
  Uint8List encode() {
    ...
    builder.add(VariableLengthInteger(type, 0).encode()); // Encode type as VLI (length will be determined by value)
    builder.add(VariableLengthInteger(streamId, 0).encode());
    builder.add(VariableLengthInteger(applicationProtocolErrorCode, 0).encode());
...
class RetireConnectionIdFrame extends QuicFrame {
  ...
  factory RetireConnectionIdFrame.parse(Uint8List bytes, int offset) {
    ...
    if (frameTypeVli.value != 0x09) {
      throw FormatException('Invalid frame type for RetireConnectionIdFrame: 0x${frameTypeVli.value.toRadixString(16)}');
    }
    currentOffset += frameTypeVli.byteLength;

    final VariableLengthInteger sequenceNumberVli = VariableLengthInteger.decode(bytes, currentOffset);
    // currentOffset += sequenceNumberVli.byteLength; // Not needed for parsing

    return RetireConnectionIdFrame(
      sequenceNumber: sequenceNumberVli.value,
    );
  }
...
class StopSendingFrame extends QuicFrame {
  ...
  factory StopSendingFrame.parse(Uint8List bytes, int offset) {
    ...
    if (frameTypeVli.value != 0x05) {
      throw FormatException('Invalid frame type for StopSendingFrame: 0x${frameTypeVli.value.toRadixString(16)}');
    }
    currentOffset += frameTypeVli.byteLength;
...
class StreamDataBlockedFrame extends QuicFrame {
  ...
  factory StreamDataBlockedFrame.parse(Uint8List bytes, int offset) {
    ...
    if (frameTypeVli.value != 0x0F) {
      throw FormatException('Invalid frame type for StreamDataBlockedFrame: 0x${frameTypeVli.value.toRadixString(16)}');
    }
    currentOffset += frameTypeVli.byteLength;
    ...
    final VariableLengthInteger maximumStreamDataVli = VariableLengthInteger.decode(bytes, currentOffset);
...
class StreamFrame extends QuicFrame {
  ...
  factory StreamFrame.parse(Uint8List bytes, int offset) {
    ...
    if ((frameType & 0xF8) != baseType) {
      throw FormatException('Invalid frame type for StreamFrame: 0x${frameType.toRadixString(16)}');
    }
    ...
    int parsedOffset = 0;
    if (hasOffset) {
      final VariableLengthInteger offsetVli = VariableLengthInteger.decode(bytes, currentOffset);
    ...
    int? parsedLength;
    if (hasLength) {
      final VariableLengthInteger lengthVli = VariableLengthInteger.decode(bytes, currentOffset);
      parsedLength = lengthVli.value;
      currentOffset += lengthVli.byteLength;
    }

    // Remaining bytes are stream data
    final Uint8List streamData = Uint8List.fromList(bytes.sublist(currentOffset, bytes.length));

    // If length field is present, verify it matches the actual data length
    if (hasLength && parsedLength != streamData.length) {
      throw FormatException('Stream frame length mismatch. Declared: $parsedLength, Actual: ${streamData.length}');
    }
  ...
  Uint8List encode() {
    ...
    if (offset != 0) actualType |= offsetBit; // Only set OFFSET bit if offset is non-zero

    builder.add(VariableLengthInteger(actualType, 0).encode());
    builder.add(VariableLengthInteger(streamId, 0).encode());
    if (offset != 0) { // Only add offset if it's non-zero
...
class StreamsBlockedFrame extends QuicFrame {
  ...
  StreamsBlockedFrame({
    ...
    required this.maximumStreams,
  }) : super(streamType == StreamType.bidirectional ? 0x06 : 0x07); // Type 0x06 for bidirectional, 0x07 for unidirectional
  ...
  factory StreamsBlockedFrame.parse(Uint8List bytes, int offset) {
    ...
    if (frameType == 0x06) {
      ...
      throw FormatException('Invalid frame type for StreamsBlockedFrame: 0x${frameType.toRadixString(16)}');
    }
    currentOffset += frameTypeVli.byteLength;

    final VariableLengthInteger maximumStreamsVli = VariableLengthInteger.decode(bytes, currentOffset);
