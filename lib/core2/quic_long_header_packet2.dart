...
enum QuicLongHeaderType {
  ...
  final int? value; // 2-bit value for Initial, 0-RTT, Handshake, Retry. Null for Version Negotiation.
...
class LongHeaderPacket extends QuicPacket {
  ...
  final Uint8List sourceConnectionId; // Source Connection ID (0..2040 for VN, 0..160 for others)
  ...
  final Uint8List? retryIntegrityTag; // Retry Integrity Tag (128) - for Retry Packet (16 bytes)

  // Fields specific to Initial, 0-RTT, Handshake packets (and part of first byte for Retry)
  final int? reservedBits; // Reserved Bits (2) - MUST be 0 (after header protection removed)
  ...
  final int? decodedPacketNumber; // Decoded full packet number (after header protection removed)
  ...
  }) : assert(fixedBit || identifiedType == QuicLongHeaderType.versionNegotiation,
            'Fixed Bit MUST be 1 unless it is a Version Negotiation packet.'),
       assert((version == 0x00000000) == isVersionNegotiation, // Version is 0x00000000 IFF it's VN
            'Version must be 0x00000000 if isVersionNegotiation is true, and non-zero otherwise.'),
       assert(reservedBits == null || (reservedBits! >= 0 && reservedBits! <= 3), 'Reserved Bits must be 0-3'),
       assert(packetNumberLengthBits == null || (packetNumberLengthBits! >= 0 && packetNumberLengthBits! <= 3), 'Packet Number Length must be 0-3');
  ...
  factory LongHeaderPacket.parse(Uint8List datagramBytes, {int largestReceivedPn = 0}) {
    ...
    int typeSpecificBitsValue; // The raw value of the type-specific bits from byte 0

    if (isVersionNegotiation) {
      identifiedType = QuicLongHeaderType.versionNegotiation;
      // For VN, Fixed Bit MUST be 0.
      if (fixedBit) {
        throw FormatException('Malformed Version Negotiation Packet: Fixed Bit MUST be 0 (0x40 bit set).');
      }
      ...
      typeSpecificBitsValue = (firstByte & 0x3F); // Capture all 6 bits for VN (should be 0)
    } else {
      // Normal Long Header Packets (Initial, 0-RTT, Handshake, Retry)
      final int longPacketTypeValue = (firstByte & 0x30) >> 4; // Extract 2 bits for type
      final QuicLongHeaderType? resolvedType = QuicLongHeaderType.fromValue(longPacketTypeValue);
      if (resolvedType == null) {
        throw FormatException('Unknown Long Packet Type: 0x${longPacketTypeValue.toRadixString(16)} for non-VN packet. First byte: 0x${firstByte.toRadixString(16)}');
      }
      identifiedType = resolvedType;
      // For these types, Fixed Bit MUST be 1.
      if (!fixedBit) {
         throw FormatException('Malformed Long Header Packet (type $identifiedType): Fixed Bit MUST be 1 (0x40 bit unset).');
      }
      typeSpecificBitsValue = (firstByte & 0x0F); // Only 4 bits for these types (Reserved Bits + PN Length)
    }

    // Common CID parsing (applies to all Long Header Packet types, including VN)
    if (datagramBytes.length < offset + 1) {
      throw FormatException('Datagram too short to read Destination Connection ID Length.');
    }
    ...
    if (version == 0x00000001 && destCidLength > 20 && !isVersionNegotiation) {
       throw FormatException('Destination Connection ID Length ($destCidLength) exceeds 20 bytes for QUIC Version 1 non-VN packet.');
    }
    if (datagramBytes.length < offset + destCidLength) {
      throw FormatException('Datagram too short to read Destination Connection ID ($destCidLength bytes).');
    }
    final Uint8List destinationConnectionId = Uint8List.fromList(datagramBytes.sublist(offset, offset + destCidLength));
    offset += destCidLength;

    if (datagramBytes.length < offset + 1) {
      throw FormatException('Datagram too short to read Source Connection ID Length.');
    }
    final int sourceCidLength = byteData.getUint8(offset++);
    // For QUIC V1, max CID len is 20. VN allows up to 2040.
    if (version == 0x00000001 && sourceCidLength > 20 && !isVersionNegotiation) {
      throw FormatException('Source Connection ID Length ($sourceCidLength) exceeds 20 bytes for QUIC Version 1 non-VN packet.');
    }
    if (datagramBytes.length < offset + sourceCidLength) {
      throw FormatException('Datagram too short to read Source Connection ID ($sourceCidLength bytes).');
    }
    final Uint8List sourceConnectionId = Uint8List.fromList(datagramBytes.sublist(offset, offset + sourceCidLength));
    ...
    if (isVersionNegotiation) {
      ...
      supportedVersions = (offset < datagramBytes.lengthInBytes)
          ...
          : null;
      offset = datagramBytes.lengthInBytes; // All remaining bytes consumed by VN payload
      ...
      if (datagramBytes.length < offset + 1) {
         throw FormatException('Datagram too short to read Token Length field (VLQ header).');
      }
      final tokenLengthEntry = QuicVariableLengthInteger.decode(datagramBytes, offset);
      tokenLength = tokenLengthEntry.key;
      offset += tokenLengthEntry.value;

      if (tokenLength > 0) {
        if (datagramBytes.length < offset + tokenLength) {
          throw FormatException('Datagram too short to read Token ($tokenLength bytes).');
        }
        token = Uint8List.fromList(datagramBytes.sublist(offset, offset + tokenLength));
        offset += tokenLength;
      } else {
      ...
      reservedBits = (typeSpecificBitsValue & 0x0C) >> 2; // Extract from the 4 type-specific bits
      packetNumberLengthBits = typeSpecificBitsValue & 0x03; // Extract from the 4 type-specific bits

      if (datagramBytes.length < offset + 1) {
         throw FormatException('Datagram too short to read Length field (VLQ header).');
      }
      final lengthEntry = QuicVariableLengthInteger.decode(datagramBytes, offset);
      ...
      if (datagramBytes.lengthInBytes - offset < pnBytes) {
        throw FormatException('Packet number length ($pnBytes bytes) exceeds remaining packet data for Initial packet. Remaining: ${datagramBytes.lengthInBytes - offset}');
      }
      packetNumberRaw = Uint8List.fromList(datagramBytes.sublist(offset, offset + pnBytes));
      offset += pnBytes;

      decodedPacketNumber = QuicPacketNumber.decode(packetNumberRaw, packetNumberLengthBits, largestReceivedPn);

      final int expectedPayloadEnd = offset + (length - pnBytes); // Length field includes PN and Payload
      if (expectedPayloadEnd > datagramBytes.lengthInBytes) {
        throw FormatException('Declared packet length ($length) exceeds actual datagram size for Initial packet. Expected end: $expectedPayloadEnd, Actual length: ${datagramBytes.lengthInBytes}');
      }
      packetPayload = Uint8List.fromList(datagramBytes.sublist(offset, expectedPayloadEnd));
      offset = expectedPayloadEnd;

    } else if (identifiedType == QuicLongHeaderType.zeroRtt || identifiedType == QuicLongHeaderType.handshake) {
      ...
      final int pnBytes = packetNumberLengthBits + 1;
      if (datagramBytes.lengthInBytes - offset < pnBytes) {
        throw FormatException('Packet number length ($pnBytes bytes) exceeds remaining packet data for ${identifiedType.name} packet. Remaining: ${datagramBytes.lengthInBytes - offset}');
      }
      packetNumberRaw = Uint8List.fromList(datagramBytes.sublist(offset, offset + pnBytes));
      offset += pnBytes;
      ...
      final int expectedPayloadEnd = offset + (length - pnBytes); // Length field includes PN and Payload
      if (expectedPayloadEnd > datagramBytes.lengthInBytes) {
        throw FormatException('Declared packet length ($length) exceeds actual datagram size for ${identifiedType.name} packet. Expected end: $expectedPayloadEnd, Actual length: ${datagramBytes.lengthInBytes}');
      }
      packetPayload = Uint8List.fromList(datagramBytes.sublist(offset, expectedPayloadEnd));
      offset = expectedPayloadEnd;
      ...
      if (datagramBytes.length < offset + 1) {
         throw FormatException('Datagram too short to read Retry Token Length field (VLQ header).');
      }
      final retryTokenLengthEntry = QuicVariableLengthInteger.decode(datagramBytes, offset);
      ...
      if (datagramBytes.length < offset + retryTokLen) {
        throw FormatException('Datagram too short to read Retry Token ($retryTokLen bytes).');
      }
      retryToken = Uint8List.fromList(datagramBytes.sublist(offset, offset + retryTokLen));
      offset += retryTokLen;

      // Retry Integrity Tag (fixed 16 bytes)
      const int retryTagLength = 16;
      if (datagramBytes.length < offset + retryTagLength) {
        throw FormatException('Datagram too short to read Retry Integrity Tag ($retryTagLength bytes).');
      }
      retryIntegrityTag = Uint8List.fromList(datagramBytes.sublist(offset, offset + retryTagLength));
  ...
  Uint8List toBytes() {
    ...
    if (isVersionNegotiation) {
        ...
        firstByte |= (identifiedType.value! << 4) & 0x30; // Long Packet Type (0x03)
        ...
        firstByte &= 0xF0; // Clear the lower 4 bits (0x0F) - effectively setting 'Unused' to 0.
    } else {
        // For Initial, 0-RTT, Handshake, use the identifiedType's value for the 2 bits.
        firstByte |= (identifiedType.value! << 4) & 0x30; // Long Packet Type (2 bits)
    ...
    if (isVersionNegotiation) {
        ...
        // Length (i)
        if (length == null) throw StateError('Initial packet must have a length field for serialization.');
        bytes.addAll(QuicVariableLengthInteger.encode(length!));

        // Packet Number (1-4 bytes)
        if (packetNumberRaw == null) throw StateError('Initial packet must have raw packet number for serialization.');
        bytes.addAll(packetNumberRaw!);

        // Packet Payload
        if (packetPayload == null) throw StateError('Initial packet must have packet payload for serialization.');
        bytes.addAll(packetPayload!);

    } else if (identifiedType == QuicLongHeaderType.zeroRtt || identifiedType == QuicLongHeaderType.handshake) {
        // Length (i)
        if (length == null) throw StateError('${identifiedType.name} packet must have a length field for serialization.');
        bytes.addAll(QuicVariableLengthInteger.encode(length!));

        // Packet Number (1-4 bytes)
        if (packetNumberRaw == null) throw StateError('${identifiedType.name} packet must have raw packet number for serialization.');
        bytes.addAll(packetNumberRaw!);

        // Packet Payload
        if (packetPayload == null) throw StateError('${identifiedType.name} packet must have packet payload for serialization.');
        bytes.addAll(packetPayload!);

    } else if (identifiedType == QuicLongHeaderType.retry) {
        // Retry Token (..)
        if (retryToken == null) throw StateError('Retry packet must have a retry token for serialization.');
        ...
        if (retryIntegrityTag == null || retryIntegrityTag!.length != 16) {
            throw StateError('Retry packet must have a 16-byte integrity tag for serialization.');
        }
  ...
  String toString() {
    ...
    if (isVersionNegotiation) {
      supportedVersionsInfo = 'Supported Versions Length: ${supportedVersions?.length ?? 0} bytes, Supported Versions: ${supportedVersions?.map((e) => e.toRadixString(16).padLeft(2,'0')).join('')}, ';
    } else if (identifiedType == QuicLongHeaderType.initial) {
      tokenInfo = 'Token Length: $tokenLength, Token: ${token?.map((e) => e.toRadixString(16).padLeft(2, '0')).join('')}, ';
      pnInfo = 'PN Length Bits: $packetNumberLengthBits, Raw PN: ${packetNumberRaw?.map((e) => e.toRadixString(16).padLeft(2, '0')).join(' ')}, Decoded PN: $decodedPacketNumber, ';
      payloadInfo = 'Length (payload+PN): $length, Packet Payload Size: ${packetPayload?.length ?? 0} bytes, ';
    } else if (identifiedType == QuicLongHeaderType.zeroRtt || identifiedType == QuicLongHeaderType.handshake) {
      pnInfo = 'PN Length Bits: $packetNumberLengthBits, Raw PN: ${packetNumberRaw?.map((e) => e.toRadixString(16).padLeft(2, '0')).join(' ')}, Decoded PN: $decodedPacketNumber, ';
      payloadInfo = 'Length (payload+PN): $length, Packet Payload Size: ${packetPayload?.length ?? 0} bytes, ';
    } else if (identifiedType == QuicLongHeaderType.retry) {
      retryInfo = 'Retry Token Length: ${retryToken?.length ?? 0}, Retry Token: ${retryToken?.map((e) => e.toRadixString(16).padLeft(2, '0')).join('')}, Retry Integrity Tag: ${retryIntegrityTag?.map((e) => e.toRadixString(16).padLeft(2, '0')).join('')}, ';
    }
