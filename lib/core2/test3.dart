...
import 'quic_long_header_packet3.dart';
import 'quic_short_header_packet.dart'; // Import the new short header packet
...
void main() {
  ...
  // --- Testing ShortHeaderPacket ---
  print('\n--- Testing ShortHeaderPacket ---');
  final Uint8List shortHeaderDestCid = Uint8List.fromList([
    0xde,
    0xad,
    0xbe,
    0xef,
  ]); // 4-byte CID
  final Uint8List shortHeaderPayload = Uint8List.fromList([
    0x0a,
    0x0b,
    0x0c,
    0x0d,
  ]); // Small payload
  final int shortHeaderPn = 123;
  // Encode PN assuming 2-byte representation (pn_length_bits = 1)
  final Uint8List shortHeaderEncodedPn = QuicPacketNumber.encode(
    shortHeaderPn,
    100,
  ); // encoded as 2 bytes
  final int shortHeaderPnLengthBits =
      shortHeaderEncodedPn.length - 1; // Expected to be 1 for 2 bytes

  final ShortHeaderPacket shortPacket = ShortHeaderPacket(
    fixedBit: true,
    spinBit: false, // Initial spin bit
    reservedBits: 0, // Must be 0
    keyPhase: false, // Initial key phase
    packetNumberLengthBits: shortHeaderPnLengthBits,
    destinationConnectionId: shortHeaderDestCid,
    packetNumberRaw: shortHeaderEncodedPn,
    decodedPacketNumber: shortHeaderPn,
    packetPayload: shortHeaderPayload,
  );

  final Uint8List serializedShortPacket = shortPacket.toBytes();
  print(
    '\n--- Serialized Short Header Packet (${serializedShortPacket.length} bytes) ---',
  );
  printBytes(serializedShortPacket);

  print('\n--- Parsing Serialized Short Header Packet ---');
  try {
    // We must provide the expected CID length for parsing short headers
    final ShortHeaderPacket parsedShortPacket = ShortHeaderPacket.parse(
      serializedShortPacket,
      expectedDestinationConnectionIdLength: shortHeaderDestCid.length,
      largestReceivedPn: 100, // largest PN seen for this CID space
    );
    print(parsedShortPacket);
    assert(parsedShortPacket.isLongHeader == false);
    assert(parsedShortPacket.fixedBit == true);
    assert(parsedShortPacket.spinBit == false);
    assert(parsedShortPacket.reservedBits == 0);
    assert(parsedShortPacket.keyPhase == false);
    assert(parsedShortPacket.packetNumberLengthBits == shortHeaderPnLengthBits);
    assert(
      ListEquality().equals(
        parsedShortPacket.destinationConnectionId,
        shortHeaderDestCid,
      ),
    );
    assert(
      ListEquality().equals(
        parsedShortPacket.packetNumberRaw,
        shortHeaderEncodedPn,
      ),
    );
    assert(parsedShortPacket.decodedPacketNumber == shortHeaderPn);
    assert(
      ListEquality().equals(
        parsedShortPacket.packetPayload,
        shortHeaderPayload,
      ),
    );
    print('Short Header Packet parsing and comparison successful!');
  } catch (e) {
    print('Error parsing Short Header packet: $e');
  }

  // Test ShortHeaderPacket with different PN length (1-byte)
  print('\n--- Testing ShortHeaderPacket (1-byte PN) ---');
  final int shortHeaderPn2 = 50;
  final Uint8List shortHeaderEncodedPn2 = QuicPacketNumber.encode(
    shortHeaderPn2,
    0,
  ); // Should be 1 byte
  final int shortHeaderPnLengthBits2 =
      shortHeaderEncodedPn2.length - 1; // Expected to be 0 for 1 byte

  final ShortHeaderPacket shortPacket2 = ShortHeaderPacket(
    fixedBit: true,
    spinBit: true, // Spin bit set
    reservedBits: 0,
    keyPhase: true, // Key phase set
    packetNumberLengthBits: shortHeaderPnLengthBits2,
    destinationConnectionId: shortHeaderDestCid,
    packetNumberRaw: shortHeaderEncodedPn2,
    decodedPacketNumber: shortHeaderPn2,
    packetPayload: shortHeaderPayload,
  );

  final Uint8List serializedShortPacket2 = shortPacket2.toBytes();
  print(
    '\n--- Serialized Short Header Packet 2 (${serializedShortPacket2.length} bytes) ---',
  );
  printBytes(serializedShortPacket2);

  try {
    final ShortHeaderPacket parsedShortPacket2 = ShortHeaderPacket.parse(
      serializedShortPacket2,
      expectedDestinationConnectionIdLength: shortHeaderDestCid.length,
      largestReceivedPn: 0,
    );
    print(parsedShortPacket2);
    assert(parsedShortPacket2.spinBit == true);
    assert(parsedShortPacket2.keyPhase == true);
    assert(
      parsedShortPacket2.packetNumberLengthBits == shortHeaderPnLengthBits2,
    );
    assert(parsedShortPacket2.decodedPacketNumber == shortHeaderPn2);
    print('Short Header Packet (1-byte PN) parsing and comparison successful!');
  } catch (e) {
    print('Error parsing Short Header packet 2: $e');
  }
}
